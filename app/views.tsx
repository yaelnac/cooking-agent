...
export function HomeView() {
  ...
  const openCookingTab = useCallback(() => {
    window.open(COOKING_PATH, '_blank', 'noopener,noreferrer');
  }, []);

  return (
    <div className="flex flex-col">
      <Hero onTap={openCookingTab} status={status} />
...
export function CookingView() {
  ...
  const { endSession, sendUserMessage } = useConversationControls();
  ...
  const currentRecipeLabel = session.recipe?.name ?? 'Start cooking';
  const hasActiveStep = Boolean(session.step && !session.completed);

  return (
    <div className="flex min-h-screen flex-col px-5 pb-24 pt-5">
      <div className="flex items-center justify-between gap-3">
        <button
          onClick={endSession}
          aria-label="End session"
          className="grid h-9 w-9 cursor-pointer place-items-center rounded-full border border-line bg-paper text-ink-soft transition hover:border-ink-faint hover:text-ink"
        >
        ...
        {status !== 'connected' && status !== 'disconnected' ? (
          ...
          <span className="truncate text-[11px] font-medium uppercase tracking-[0.18em] text-ink-faint">
            {currentRecipeLabel}
          </span>
        )}
      </div>

      <VoiceOrb status={status} compact={!hasActiveStep && !session.recipe} />
      <StepCard step={session.step} completed={session.completed} recipe={session.recipe} />
      {hasActiveStep && (
        <CommandChips onSay={sendUserMessage} />
      )}
...
function VoiceOrb({
  ...
  return (
    <div className={`${compact ? 'my-5' : 'my-7'} flex flex-col items-center gap-3`}>
      <div
      ...
      >
        ...
        <span className="relative z-10">
          {isListening ? <WaveBars /> : (
            <MicIcon className={`${compact ? 'h-6 w-6' : 'h-8 w-8'} text-paper`} />
          )}
        </span>
      </div>
      <p className="rounded-full border border-line bg-paper/70 px-3 py-1 text-xs font-medium text-ink-soft">
        {label}
      </p>
    </div>
  );
...
function CommandChips({ onSay }: { onSay: (text: string) => void }) {
  const commands: Array<{ label: string; say: string; help: string }> = [
    { label: 'Done', say: 'done', help: 'Go to the next step' },
    { label: 'Repeat', say: 'repeat that', help: 'Hear the current step again' },
    { label: '+5 min', say: 'add 5 minutes', help: 'Extend the timer' },
    { label: 'Macros', say: 'how much protein?', help: 'Check macros' },
  ];
  return (
    <div className="mt-5 flex flex-col items-center gap-2">
      <span className="text-[10px] font-medium uppercase tracking-[0.22em] text-ink-faint">
        Quick moves
      </span>
      <div className="flex flex-wrap items-center justify-center gap-2">
        {commands.map((c) => (
          <button
            ...
            aria-label={c.help}
            title={c.help}
            className="cursor-pointer rounded-full border border-line bg-paper px-3 py-1.5 text-xs font-medium text-ink-soft shadow-[0_8px_18px_-16px_rgba(26,20,16,0.35)] transition hover:-translate-y-0.5 hover:border-ink-faint hover:bg-cream hover:text-ink"
          >
            {c.label}
          </button>
        ))}
...
function StepCard({
  ...
  if (completed) {
    return (
      <div className="anim-fade-up mt-2 flex flex-col items-center gap-3 rounded-3xl border border-forest/20 bg-forest-soft p-6 text-center shadow-[0_20px_45px_-35px_rgba(31,77,60,0.55)]">
        <span className="grid h-11 w-11 place-items-center rounded-full bg-forest text-paper shadow-[0_12px_24px_-16px_rgba(31,77,60,0.9)]">
          <CheckIcon className="h-5 w-5" />
        </span>
        <h3 className="font-display text-2xl tracking-tight text-forest">
          Ready to plate.
        </h3>
        <p className="text-sm text-forest/80">
          {recipe?.proteinLabel
            ? `${recipe.proteinLabel} protein, no guesswork.`
            : 'Nice work. Take the first bite while it is hot.'}
        </p>
      </div>
    );
  }
  ...
  if (!step) {
    return (
      <div className="anim-fade-up mx-auto mt-1 flex max-w-xs flex-col items-center gap-2 text-center">
        <h2 className="font-display text-2xl leading-tight tracking-tight text-ink">
          What should we make?
        </h2>
        <p className="text-sm leading-relaxed text-ink-faint">
          Say a dish, an ingredient, or the kind of meal you want.
        </p>
        <div className="mt-2 flex flex-wrap justify-center gap-2">
          {['chicken rice', 'quick breakfast', 'high-protein dinner'].map((hint) => (
            <span
              key={hint}
              className="rounded-full border border-line bg-paper/60 px-2.5 py-1 text-[11px] text-ink-faint"
            >
              {hint}
            </span>
          ))}
  ...
  return (
    <div className="anim-fade-up mt-2 flex flex-col gap-5 rounded-3xl border border-line bg-paper p-6 shadow-[0_18px_45px_-34px_rgba(26,20,16,0.35)]">
      <div className="flex items-center gap-3">
        <span className="shrink-0 text-[11px] font-semibold uppercase tracking-[0.18em] text-terracotta">
          Step {step.index} <span className="text-ink-faint">of {step.total}</span>
        </span>
      ...
      <p className="font-display text-[26px] leading-tight tracking-tight md:text-[32px]">
        {step.text}
      </p>
    </div>
  );
    ...
    <div className="mt-5 flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[11px] text-ink-faint">
          {timers.filter((t) => !t.isDone).length} active
        </span>
      </div>
...
function TimerRow({
  ...
  return (
    <div
      className={`anim-fade-up flex items-center justify-between gap-3 rounded-2xl border p-3 shadow-[0_10px_26px_-24px_rgba(26,20,16,0.35)] ${
        timer.isDone
